...
## Option: Leaving the root filesystem on NVMe
...
If you want the firmware booting on eMMC or microSD mounts NVMe partition as root filesystem, you have to append the boot args into `/boot/env_k1-x.txt`.
...
Q: It does not work with other Bianbu release's boot part.

A: For other Bianbu release version, try checking the U-Boot environment variables and the related booting commands.

### Help: How to inspect U-Boot environment variables so that I can set correct boot args?
...
>
> For BPi-F3 board:
> Press and hold the [DFU] and [RST] buttons in order to enter firmware update mode.
